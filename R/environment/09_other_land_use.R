...
## extract all of the land_use types in the each county
...
land_use_categories <- read.csv("clean_data/land_use/land_use_cats.csv") %>% 
  ...
  mutate(land_use_collapsed = case_when(str_detect(land_use, "Developed") ~ "Developed",
                                        land_use %in% c("Deciduous Forest", "Evergreen Forest", "Mixed Forest", "Woody Wetlands", 
...
percent_county_land_use %>% 
  ...
  mutate(state = str_extract(state_county, "\\d\\d")) %>% 
  filter(state == "04") %>% 
  ggplot() +
  geom_line(aes(x = year, y = percent_county, group = state_county))

### not much change at this scale
...
### check correlations with canag, or pest
...
nat_pest_plot_all <- list()

nat_pest_correlation_all <- list()

nat_canag_plot_all <- list()
...
for(f in fam){
  ...
  nat_canag_plot_all[[f]] <- canag_percent_nat %>% as.data.frame() %>% 
    pivot_longer(names_to = "year", values_to = "nat", -canag) %>% 
    ggplot(aes(x = canag, y = nat, colour = str_remove(year, "yr"))) + 
    geom_point() +
    theme_cowplot() +
    ggtitle(str_replace(f, "_", " ")) +
    xlab("Proportion of the county that is \n animal pollinated agriculture")+
    ylab("Proportion of county that is natural areas") +
  ...
  nat_pest_plot_all[[f]] <- natural_pest_fam %>% 
    ggplot(aes(x = pest, y = percent_county, colour = year)) + 
    geom_point() +
    theme_cowplot() +
    ggtitle(str_replace(f, "_", " ")) +
    xlab("Pesticide use")+
...
##### check which land use has changed the most #####
...
county_land_use <- land_use_all_cats %>% 
  group_by(state_county, year, land_use) %>% 
  summarise(land_use_total = sum(Freq))
...
percent_county_land_use <- county_land_use %>% 
  ...
  mutate(site = paste0("s_", state_county)) %>% 
  ungroup() %>% 
  dplyr::select(site, year, land_use, percent_county) %>% 
  pivot_wider(names_from =  year, values_from = percent_county) %>% 
  mutate(change = `2008` - `2001`, net_change = abs(`2008` - `2001`)) 

percent_county_land_use %>% 
  ggplot(aes(x = land_use, y = change)) +
  geom_jitter()
