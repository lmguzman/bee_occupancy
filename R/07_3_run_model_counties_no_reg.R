...
run_model <- function(year_range, model, family){
  ...
  saveRDS(res, paste0("model_outputs/res_counties_", paste0(year_range, collapse = "_"), "_",model,"_", family,  ".rds"))
  
  
  res.summary <-  add.summary(res)
  
  
  saveRDS(res.summary, paste0("model_outputs/res.summary_counties_", paste0(year_range, collapse = "_"), "_",model,"_",  ".rds"))
  
}

run_model(c(1995, 2014), 'ms_era_1_area', "ALL")

run_model(c(1995, 2014), 'ms_env_US_area', "ALL")

run_model(c(1995, 2014), 'ms_env_era_area', "ALL")
