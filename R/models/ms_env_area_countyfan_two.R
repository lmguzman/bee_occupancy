model.jags <- function() {
  ...
  ## Interaction between pest and can
  
  mu.psi.int.pest.can ~ dnorm(0,0.01)
  sigma.psi.int.pest.can ~ dunif(0,10)
  tau.psi.int.pest.can <- 1/(sigma.psi.int.pest.can*sigma.psi.int.pest.can)
  
  for(sp in 1:nsp){
    psi.int.pest.can[sp] ~ dnorm(mu.psi.int.pest.can, tau.psi.int.pest.can)
  }
    ...
    for(yr in 1:nyr) {
      for(site in 1:nsite){
        ...
        logit(psi[site,yr,sp]) <-
          ...
          psi.int.pest.can[sp]*countanimal[site]*pesticide1[site,yr] 
        # 
        ## detection
        logit(p[site,yr,sp]) <-
          mu.p.0 +
